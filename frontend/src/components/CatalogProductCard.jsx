...
export function CatalogProductCard({ product }) {
    ...
    return (
        <article className="card group h-full overflow-hidden border border-base-300 bg-base-100 shadow-md transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-xl">
            ...
            <div className="card-body grow gap-3 p-5 text-right">
                <Link
                    to={`/product/${product.slug}`}
                    className="card-title text-justify line-clamp-2 text-lg transition group-hover:text-primary"
                >
                    {product.name}
                </Link>
                <p className="line-clamp-2 text-justify text-sm leading-relaxed text-base-content/70">
                    {product.description}
                </p>
                <div className="card-actions mt-auto items-center justify-between border-t border-base-200 pt-4">
                    <span className="text-lg font-samimBold tabular-nums text-base-content">
                    ...
                    >
                        ...
                        <span className="font-samimBold">خرید</span>
                    </button>
                </div>
            </div>
        </article>
