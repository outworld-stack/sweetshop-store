...
export async function polarWebhookHandler(req: Request, res: Response) {
  ...
  try {
    ...
    // ✅ تغییر ۱: پیشوند whsec_ رو حذف نکن، کتابخانه خودش هندل می‌کنه
    const wh = new Webhook(env.POLAR_WEBHOOK_SECRET);
    ...
    // ✅ تغییر ۲: مستقیماً req.body (که از نوع Buffer هست) رو پاس بده
    ...
    // پارس کردن نتیجه‌ی تایید شده
    const rawBody = verifiedPayload instanceof Buffer
      ? verifiedPayload.toString("utf-8")
