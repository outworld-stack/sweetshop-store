...
export async function createStreamToken(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!localUser) {
            res.status(503).json({ error: 'Account no Sync yet' });
            return;
        }
        ...
        const name = streamChatDisplayName(localUser.role, localUser.displayName ?? fullName ?? clerkUser.username, localUser.email);
